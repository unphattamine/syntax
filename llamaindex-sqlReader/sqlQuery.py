...
def chat_to_sql(question: str | list[str], tables: list[str] | None = None, synthesize_response: bool = True):
    ...
    if duration is not None:
        print(Fore.RESET + f"\nQuery took me: {duration:.2f} seconds\n")

# Starting the show, connection to database
try:
    # Create SQLAlchemy engine and inspect table names
    print("\nWelcome to Syntax Q&A dbSQL chat.")
    print("\nCreating DB connection...")
    engine = create_engine(connection_uri)
    sql_database = SQLDatabase(engine)
    inspector = inspect(engine)
    table_names = inspector.get_table_names()
    print("Please wait, I'm loading tables", table_names, "of", db_name, "database...\n")
    # Configure LLM settings
    Settings.llm = OpenAI(temperature=0.5, model="gpt-3.5-turbo-16k")
except Exception as ex:
    errConn = "I was not able to connect to MySQL database"
    print(errConn)
